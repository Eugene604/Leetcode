...
class Solution746 {
    ...
    public int minCostClimbingStairs(int[] cost) {
    	...
    	//step 1, fill in last two directly as they represent the min cost of their own    	
    	minCostArr[i] = cost[i];
    	i--;
    	minCostArr[i] = cost[i];
    	i--;
    	
    	//step 2, iterate back until index 0
    	while (i>=0) {
    		minCostArr[i] = Math.min(minCostArr[i+1],minCostArr[i+2]) + cost[i];
    		i--;
    	}//end while
...
class Solution746_v2 {
	private int[] costArr;
	private int globalMinCost;
	
    public int minCostClimbingStairs(int[] cost) {
    	this.costArr = cost;
    	this.globalMinCost = 500000;
    	getCost(0,0);
    	getCost(1,0);
    ...
    /**
     * recursively traverse through array
     * Precondition:
     * required data structures and variables are set - costArr, globalMinCost
     * Postcondition:
     * globalMinCost updated 
     * @param currInx - int, the current index number
     * @param currCost - int, the cost accumulated so far
     */
    private void getCost(int currInx, int currCost) {
    	
    	//base case 1, current path already exceeded known global minimum cost, stops traversing the array
    	if (currCost >= globalMinCost) {
    		return;
    	}//fi
    	
		currCost +=  costArr[currInx];		
		
    	//base case 2, arrived end of array
    	if (currInx == costArr.length-1 || currInx == costArr.length-2) {
    		globalMinCost = Math.min(globalMinCost, currCost);
    		return;
    	}//fi
    		  	
		getCost(currInx+2,currCost);  
		getCost(currInx+1,currCost);    		  				
