...
public class Prob1535 {
	static int[] nums0 = {2,1,3,5,4,6,7};
	static int[] nums1 = {3,2,1};
	...
	private static void test() {
		int[] testArr;
		Solution1535 solObj = new Solution1535();
		int k;
		
		testArr = nums0;
		k = 2;
		System.out.println("Arr: " + Arrays.toString(testArr));
		System.out.println("Ans: " + solObj.getWinner(testArr,k));
		/*
...
class Solution1535 {
    public int getWinner(int[] arr, int k) {
    	int currFinalist = arr[0];
    	int roundRemain = k;
    	for (int i = 1; i < arr.length; i++) {   
    		if (roundRemain == 0){
     			return currFinalist;
     		} //fi
    		if (arr[i] > currFinalist) {
    			currFinalist = arr[i];
    			roundRemain = k-1;
    		} else {
    			roundRemain--;
